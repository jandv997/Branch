  ...
  return (
    <PublicChrome>
      <main className="mx-auto max-w-4xl px-5 py-16">
        <PatentBadge />
        <h1 className="mt-4 font-display text-4xl tracking-tight">Compensation</h1>
        <p className="mt-3 text-ink-500">
          These rules are generated from the same backend policy module that posts ledger rows. The UI does not invent payouts.
        </p>
        <section className="mt-10 space-y-6 text-sm text-ink-700">
          <div className="border border-ink-900/12 bg-paper-50 p-5">
            <h2 className="font-display text-2xl">Daily credits</h2>
          ...
          <div className="border border-ink-900/12 bg-paper-50 p-5">
            <h2 className="font-display text-2xl">Referral tree (3 levels, paid once per qualifying event)</h2>
            <p className="mt-2 font-mono text-copper">
              L1 {cfg.tree.l1Bps / 100}% / L2 {cfg.tree.l2Bps / 100}% / L3 {cfg.tree.l3Bps / 100}% = {(cfg.tree.l1Bps + cfg.tree.l2Bps + cfg.tree.l3Bps) / 100}%
            </p>
            <ul className="mt-3 list-disc space-y-1 pl-5 text-ink-500">
          ...
          <div className="border border-ink-900/12 bg-paper-50 p-5">
            <h2 className="font-display text-2xl">PSV / TV</h2>
          ...
          <div className="border border-ink-900/12 bg-paper-50 p-5">
            <h2 className="font-display text-2xl">Fast Start (days 1–14 from activation)</h2>
          ...
          <div className="border border-ink-900/12 bg-paper-50 p-5">
            <h2 className="font-display text-2xl">Why tree might not pay</h2>
            <ul className="mt-2 list-disc space-y-1 pl-5 text-ink-500">
              {Object.entries(WHY_NOT_PAID_COPY).map(([k, v]) => (
                <li key={k}>
                  <span className="font-mono text-copper">{k}</span> — {v}
                </li>
              ))}
            </ul>
          </div>
