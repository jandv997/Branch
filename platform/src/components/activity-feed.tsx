...
export function ActivityFeed({
  ...
  return (
    <ol className="space-y-2">
      {items.map((i) => {
        const inner = (
          <div className="glass flex items-start justify-between gap-4 rounded-xl p-4">
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <span className="rounded border border-cyan/30 px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-widest text-cyan">
                  {KIND_LABEL[i.kind]}
                </span>
                <span className="font-mono text-[10px] text-slate-500">{new Date(i.at).toISOString().replace("T", " ").slice(0, 19)}</span>
              </div>
              <div className="mt-1 truncate text-sm text-slate-100">{i.title}</div>
