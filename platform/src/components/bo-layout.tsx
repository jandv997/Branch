...
export function UserBoLayout({ children }: { children: ReactNode }) {
  ...
  if (!me.data) return <div className="p-10 text-sm text-slate-500">Authenticating…</div>;
...
export function AdminBoLayout({ children }: { children: ReactNode }) {
  ...
  if (!me.data || me.data.role === "USER") return <div className="p-10 text-sm text-slate-500">Authenticating…</div>;
