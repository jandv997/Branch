...
if (!$exp || strtotime($exp) < time()) {
    ...
    exit;
		...
		if (!$payWallet) {
			...
			<script>
				notif({
					msg: "<b>Select a payment wallet</b><br/> Choose a direct-deposit wallet to continue.",
					width: 280,
					position: "center",
					type: "warning"
				});
		...
		sendInvoiceEmail($rows['email'], $name, $wallet, $orderId, $date);

		if ($addinvestment) {
			?>
			<script>
				location = "fund?currency=<?php echo urlencode($currency); ?>&orderid=<?php echo urlencode($orderId); ?>&name=<?php echo urlencode($name); ?>"
			</script>
			<?php
		}
